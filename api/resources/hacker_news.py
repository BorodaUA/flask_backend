...
from api.models.hn_db import HackerNews_TopStories, HackerNews_TopStories_Comments
...
class HackerNews_TopStories_Story_Resource(Resource):
    ...
    def post(cls, story_id):
        ...
        """
        try:
            incoming_story_id = story_schema.load(request.get_json())
        except ValidationError as err:
            return err.messages, 400
        ...
        story = HackerNews_TopStories.query.filter(
            HackerNews_TopStories.item_id == incoming_story_id["story_id"],
            ).order_by(HackerNews_TopStories.parse_dt).first()
...
class HackerNews_TopStories_Story_Comments_Resource(Resource):
    ...
    def post(cls, story_id):
        ...
        comments = HackerNews_TopStories_Comments.query.filter(
            HackerNews_TopStories_Comments.parent == incoming_story_id["story_id"]
        ).order_by(desc(HackerNews_TopStories_Comments.parse_dt)).all()
