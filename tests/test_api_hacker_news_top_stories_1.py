...
def test_no_json_data_and_content_type_pagination_hacker_news_top_stories(client):
    """
    test pagination 1
    """
    insert_in_table(test_row, "hacker_news_top_stories")
    response = client.post("/api/hacker_news/top_stories/1",)
    response = json.loads(response.data)
    assert {"_schema": ["Invalid input type."]} == response


def test_no_json_data_hacker_news_top_stories(client):
    """
    test pagination 2
    """
    response = client.post(
        "/api/hacker_news/top_stories/1",
        data=json.dumps({}),
...
def test_empty_table_hacker_news_top_stories(client):
    """
    test pagination 3
    """
    response = client.post(
    ...
    response = json.loads(response.data)
    assert "No top_stories in this table" == response["message"]


def test_pagination_hacker_news_top_stories(client):
    """
    test pagination 4
    """
    insert_in_table(test_row, "hacker_news_top_stories")
...
def test_no_data_story_page_hacker_news_top_stories(client):
    """
    test story 1
    """
    insert_in_table(test_row, "hacker_news_top_stories")
    response = client.post(f"/api/hacker_news/top_stories/story/{test_row['item_id']}",)
    response = json.loads(response.data)
    assert {"_schema": ["Invalid input type."]} == response


def test_no_story_id_story_page_hacker_news_top_stories(client):
    """
    test story 2
    """
    insert_in_table(test_row, "hacker_news_top_stories")
    response = client.post(
        f"/api/hacker_news/top_stories/story/{test_row['item_id']}",
        data=json.dumps({}),
        content_type="application/json",
    )
    response = json.loads(response.data)
...
def test_story_page_hacker_news_top_stories(client):
    """
    test story 3
    """
    insert_in_table(test_row, "hacker_news_top_stories")
...
def test_no_data_story_page_comments_hacker_news_top_stories(client):
    """
    test story comments 1
    """
    insert_in_table(test_row, "hacker_news_top_stories")
...
def test_story_page_comments_hacker_news_top_stories(client):
    """
    test story comments 2
    """
    insert_in_table(test_row, "hacker_news_top_stories")
    insert_in_table(test_comment_row, "hacker_news_top_stories_comments")
    response = client.post(
