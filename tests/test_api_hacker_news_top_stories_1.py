...
def test_hn_top_stories_get_pagenumber_is_valid(client):
    """
    test /api/hackernews/topstories/?pagenumber=N endpoint
    with pagenumber is a valid number
    """
...
def test_hn_top_stories_story_id_valid(client):
    ...
    """
    test_data = client.application.config['test_data']
    response = client.get(f"/api/hackernews/topstories/{test_data['hn_id']}",)
    response = json.loads(response.data)
